                ...
                @foreach($categories as $key=>$category)
                    <tr>
                        ...
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">
                                @if($category->image)
                                    <img src="{{ Storage::url($category->image) }}" alt="{{ $category->name }}" class=w-5 object-cover rounded">
                                @else
                        ...
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            ...
                            <flux:dropdown>
                                ...
                                <flux:menu>
                                    <flux:menu.item icon="pencil" href="{{ route('kategori.edit', $category->id) }}">Edit</flux:menu.item>
                                    ...
                                    <form id="delete-form-{{ $category->id }}" action="{{ route('kategori.destroy', $category->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </flux:menu>

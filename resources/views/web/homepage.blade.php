...
<x-layout>
    ...
    <!-- Carousel -->
    <div id="promo-carousel" class=" row row-cols-2 row-cols-sm-3 row-cols-md-5 g-4 carousel slide mb-5 py-5" data-bs-ride="carousel" data-bs-interval="3000">
    ...
    <div class="category-scroll1 d-flex flex-row gap-4 ">
        @for ($i = 0; $i < 100; $i++)
            @foreach ($categories as $category)
            @php
            $promos=[ "Eksplor koleksi terbaru dari $category->name hanya di sini!" , "Ngaku fans $category->name ? Lihat yang terbaru yuk!" , "Produk $category->name terkini yang wajib kamu lihat." , "Bikin harimu makin lengkap bareng $category->name." , "Baru datang! $category->name siap bikin kamu tampil beda."
            ];
            @endphp
            <div class="card text-center flex-shrink-0 hover-shadow" style="width: 15rem; height: 13rem;">
            <div class="card-body">
